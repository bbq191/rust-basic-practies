...
pub async fn get_course_detail_db(pool: &PgPool, teacher_id: i32, course_id: i32) -> Course {
    let row = sqlx::query!(
        r#"select id, teacher_id,name,time from courses where teacher_id = $1 and id = $2"#,
        teacher_id,
        course_id
    )
    .fetch_one(pool)
...
pub async fn post_new_course_db(pool: &PgPool, new_course: Course) -> Course {
    let row = sqlx::query!(
        r#"insert into courses(id, teacher_id,name) values($1,$2,$3) returning id,teacher_id, name,time"#,
        new_course.id,
